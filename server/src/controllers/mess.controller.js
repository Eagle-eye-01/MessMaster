...
exports.create = async (req, res, next) => {
  try {
    const { name, capacity, established, phone, address } = req.body;
    const mess = await Mess.create({ name, capacity, established, phone, address, adminUserId: req.user._id });

    await User.findByIdAndUpdate(req.user._id, { messId: mess._id, isSetupComplete: false });
